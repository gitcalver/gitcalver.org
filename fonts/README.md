...
`src/` holds the upstream **TrueType** (`glyf`) builds from
...
(`src/OFL.txt`). IBM Plex has no variable build, so we ship static weights: Sans
Text (450), Medium (500), SemiBold (600), Bold (700); Mono Regular (400), Medium
(500), SemiBold (600).

We vendor the TrueType (`glyf`) builds, not the CFF `.otf` builds, on purpose:
iOS Lockdown Mode (Safari 26+) runs web fonts through a memory-safe parser that
rejects CFF's charstring interpreter, so a CFF subset silently falls back to the
system serif. The `glyf` outlines pass the parser and render normally.
