...
> **Erratum (nonnormative).** Version 0.1’s “HEAD relationship” test uses
> ordinary ancestry to decide whether a target is on the default branch. That
> test also admits a commit reachable only through a merge commit’s second
> parent, even though calculation and reverse lookup traverse only the
> first-parent chain. Such a target can be reported as clean while being absent
> from the chain used for reverse lookup, so the 1:1 and reversibility
> guarantees in this document do not hold for it. Implementations should require
> exact first-parent membership for a clean version. This notice records the
> defect; it does not change the normative requirements of version 0.1.

> **Erratum (nonnormative).** Version 0.1’s “Strictly increasing” guarantee
> assumes committer dates are non-decreasing along the first-parent chain, and
> derives `N` as a count of consecutive first-parent commits sharing a date.
> That count is not robust to how a merge was invoked: merging the default
> branch into a feature branch and then fast-forwarding the default branch onto
> that merge commit reparents the default branch’s own same-date commits behind
> the feature branch’s second-parent commits. All commits remain reachable and
> no committer date decreases, yet the default branch tip’s first-parent commit
> count—and therefore its version—can decrease. Implementations should count all
> commits reachable from the target through any parent that share its date, not
> merely those consecutive on the first-parent chain; this count does not
> decrease under this topology. This notice records the defect; it does not
> change the normative requirements of version 0.1.
